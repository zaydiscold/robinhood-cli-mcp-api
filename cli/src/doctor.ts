...
export function runDoctor(root: string, env: NodeJS.ProcessEnv = process.env) {
  ...
  if (!existsSync(mcpDist)) add("mcp-build", "warn", "MCP dist is missing; build and reload the server");
  else if (existsSync(mcpSource) && statSync(mcpSource).mtimeMs > statSync(mcpDist).mtimeMs) add("mcp-build", "fail", "MCP source is newer than dist; build then reload MCP");
  else add("mcp-build", "pass", "MCP dist is current with source; reload running clients after pulls");
