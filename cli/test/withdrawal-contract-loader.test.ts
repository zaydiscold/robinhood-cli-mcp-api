...
const capture = (postBody = body) => `${JSON.stringify({
  schema: 1,
...
describe("private withdrawal contract loader", () => {
  ...
  it("refuses a cross-source, cross-destination, or cross-rail rebind", () => {
    const [contract] = loadWithdrawalContractsFromJsonl(capture());
    const input = { sourceId: "taxable-a", destinationId: "bank-a", amountUsd: "1.00", rail: "bank_standard" as const };
    expect(() => bindWithdrawalContract(contract!, { ...input, sourceId: "roth-a" })).toThrow(/source/);
    expect(() => bindWithdrawalContract(contract!, { ...input, destinationId: "bank-b" })).toThrow(/destination/);
    expect(() => bindWithdrawalContract(contract!, { ...input, rail: "bank_instant" })).toThrow(/rail/);
  });

  it("rejects malformed, non-final, or unknown-rail capture rows", () => {
    expect(loadWithdrawalContractsFromJsonl("not json\n")).toEqual([]);
    expect(loadWithdrawalContractsFromJsonl(capture({ ...body, additional_data: { entry_point: 1 } }))).toEqual([]);
    expect(loadWithdrawalContractsFromJsonl(capture({ ...body, source: { id: "taxable-a", type: "unknown" } }))).toEqual([]);
  });
});
