...
| Recurring investments | First-class list/pause/resume; **create now PROVEN** (captured 2026-06-03) | `recurring list/pause/resume`; create: `POST bonfire…/recurring_schedules/` body `{account_number,amount{amount,currency_code},frequency,investment_asset{asset_id,asset_symbol,asset_type},source_of_funds:"buying_power",start_date,ref_id}` | Pause/resume + create are double-gated writes (create makes a real schedule — delete after testing); edit/delete bodies still to capture |
