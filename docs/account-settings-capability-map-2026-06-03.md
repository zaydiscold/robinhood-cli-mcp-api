...
| Dividend reinvestment | Live read and dry-run toggle mapped | `corp_actions/drip/enrollment/{num}/` split into `GET` and `PATCH` routes | `PATCH {"drip_enrolled": true|false}` is double-gated; verify with a follow-up `GET` |
