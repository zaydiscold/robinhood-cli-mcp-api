...
| `liveWriteBlocked` | A write ran without the `ROBINHOOD_ALLOW_LIVE_WRITE=1` switch | Set the switch deliberately, inline for the one command or MCP server process; never export it in your shell profile |
