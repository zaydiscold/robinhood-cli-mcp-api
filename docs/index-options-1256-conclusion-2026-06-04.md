...
**Account context used for chain reads:** `502389919` (individual/cash).
...
```
GET options/chains/?account_number=502389919&underlying_symbol=SPX
...
1. **Account entitlement.** Chain reads returned `can_open_position: true` for account `502389919`, but actually opening index-option positions may require an options-trading approval tier / index-options entitlement. Not order-tested (read/dry-run only by instruction). Verify entitlement before assuming executability.
...
node scripts/rh-get.mjs "https://api.robinhood.com/options/chains/?account_number=502389919&underlying_symbol=SPX"
